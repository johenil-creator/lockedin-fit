#!/usr/bin/env node
/**
 * Standalone script that injects the Watch target into the pbxproj.
 * Called from the Podfile post_install hook so it runs AFTER CocoaPods.
 *
 * Usage: node plugins/injectWatchTarget.js <ios-dir>
 */
const fs = require("fs");
const path = require("path");

const iosDir = process.argv[2] || path.join(__dirname, "..", "ios");
const pbxprojPath = path.join(iosDir, "LockedInFIT.xcodeproj", "project.pbxproj");

if (!fs.existsSync(pbxprojPath)) {
  console.warn("injectWatchTarget: pbxproj not found, skipping");
  process.exit(0);
}

let pbx = fs.readFileSync(pbxprojPath, "utf8");

// Skip if embed phase already present (idempotent)
if (pbx.includes("Embed Watch Content")) {
  console.log("injectWatchTarget: Watch embed phase already present, skipping");
  process.exit(0);
}
...
function insertBefore(source, marker, content) {
  ...
  if (idx === -1) throw new Error(`injectWatchTarget: marker not found: ${marker}`);
...
// 3) Add PBXContainerItemProxy section
...
// 11) Add PBXTargetDependency section
...
// 13) Add Watch Sources and Resources build phases
pbx = insertBefore(
  pbx,
